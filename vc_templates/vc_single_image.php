...
if( isset( $curly_layer_parallax ) && wp_validate_boolean( $curly_layer_parallax ) ){


	$arte_style[] 	= ! empty( $curly_layer_1_vertical ) ? "$curly_layer_1_vertical;" : '';
	$arte_style[] 	= ! empty( $curly_layer_1_horizontal ) ? "$curly_layer_1_horizontal;" : '';
	$arte_style[] 	= ! empty( $curly_layer_1_width ) ? "width: $curly_layer_1_width;" : '';
	$arte_style[] 	= ! empty( $curly_layer_1_height ) ? "height: $curly_layer_1_height;" : '';
	$arte_style[] 	= ! empty( $curly_layer_1_z ) ? "z-index: $curly_layer_1_z;" : '';

	$arte_attr 	= $curly_layer_1_ratio  === 1 || $curly_layer_1_ratio  === '1' ? '' : " data-stellar-ratio='$curly_layer_1_ratio' ";
	$el_class 		= "parallax-image $el_class";

}

$arte_style[] = isset( $curly_max_width ) && ! empty( $curly_max_width ) ? "max-width: $curly_max_width;" : '';
$arte_style[] = isset( $curly_width ) && ! empty( $curly_width ) ? "width: $curly_width;" : '';

$arte_style = ' style="' . implode( ' ', $arte_style ) . '" ';

if( isset( $curly_hover ) && ! empty ( $curly_hover ) ){
	$image_hover = wp_get_attachment_image_src( esc_attr( $curly_hover ), empty( $img_size ) ? 'medium' : $img_size );
	$arte_attr .= ' data-img-hover="' . esc_url_raw( isset( $image_hover[0] ) ? $image_hover[0] : null ) . '" ';
}
...
switch ( $onclick ) {
	...
	case 'video_link':
		$a_attrs['class'] = 'ct-lightbox-video';
		// $link is already defined
		break;
...
$output = '
	<div class="' . esc_attr( trim( $css_class ) ) . '"' . $arte_style . $arte_attr . '>
