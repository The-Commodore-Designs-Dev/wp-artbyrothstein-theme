	...
	<div ref="carousel" class="wcs-timetable__carousel wcs-timetable__parent owl-carousel">
        ...
        <div class='responsive-image'>
          <img class='wcs-modal-call' :width="event.thumbnail_size ? event.thumbnail_size[0] : ''" :height="event.thumbnail_size ? event.thumbnail_size[1] : ''" :src="event.thumbnail" :title="event.title" v-on:click="openModal( event, options, $event )">
			...
			<div class="wcs-class__title wcs-modal-call h2" :title="event.title" v-on:click="openModal( event, options, $event )" v-html="event.title"></div>
			<div class="wcs-class__timestamp color-primary h4">
        <span class="date-starting">
					{{ event.start | moment( 'D MMM YYYY' ) }}
					<template v-if="isMultiDay(event)">
						- {{ event.end | moment( 'D MMM YYYY' ) }}
					</template>
				</span>
      </div>
			<div v-if="filter_var(options.show_excerpt)" class="wcs-class__excerpt" v-html="event.excerpt"></div>
      <div class="wcs-class__action" v-if="event.buttons.length > 0">
