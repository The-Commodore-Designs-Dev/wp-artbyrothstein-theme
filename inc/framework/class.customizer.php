...
class ArteCustomizer {
	...
	public function __construct() {
		...
		/** Slider Control */
		require_once( trailingslashit( get_template_directory() ) . 'inc/framework/class.customizer.color.php'  );
		require_once( trailingslashit( get_template_directory() ) . 'inc/framework/class.customizer.slider.php' );
		require_once( trailingslashit( get_template_directory() ) . 'inc/framework/class.customizer.typo.php'  );
		require_once( trailingslashit( get_template_directory() ) . 'inc/framework/class.slimdown.php' );

		if( class_exists( 'ArteSlimdown' ) ){
			ArteSlimdown::add_rule ('/\[br\]/', '<br>');
		}
    ...
    wp_localize_script( 'arte-customizer', 'arte_font_list', json_decode( get_option( 'arte_google_font_list' ), true ) );
	...
	public function construct_customizer( $wp_customize ) {
		...
		$options = array();
		$options = apply_filters( 'arte_theme_mods', $options );

		ArteSlimdown::add_rule ('/-----/', '\1<br>\2');
		...
		foreach ( $options as $option ) {
			...
			if( isset( $option[ 'cb' ] ) ) {
				...
				switch( $option['type'] ){
					case 'image' :
						...
						break;
					case 'color' :
						$settings_args[ 'sanitize_callback' ] = 'arte_sanitize_color_field';
					...
					case 'textarea' :
						...
						break;
					case 'select' :
						$settings_args[ 'sanitize_callback' ] = 'arte_sanitize_dropdown_field';
						break;
					case 'range' :
					...
					case 'number' :
						...
						break;
					case 'checkbox' :
						$settings_args[ 'sanitize_callback' ] = 'arte_sanitize_checkbox_field';
						break;
					default :
			...
			if( isset( $option[ 'desc' ] ) ) {
				$settings[ 'description' ] = ArteSlimdown::render( $option[ 'desc' ] );
			}
			...
			elseif( $option['type'] === 'slider' ) {
				$wp_customize->add_control(
					new arte_Slider_Control( $wp_customize, $option['id'], $settings )
				);
			}
			...
			elseif( $option['type'] === 'typo' ) {
				$wp_customize->add_control(
					new arte_Typo_Control( $wp_customize, $option['id'], $settings )
				);
			}
			...
			elseif( $option['type'] === 'sidebar' ) {
				$wp_customize->add_control(
					new arte_Sidebar_Control( $wp_customize, $option['id'], $settings )
				);
			}
			...
			elseif ( $option['type'] === 'coloor' ) {
				$wp_customize->add_control(
					new Arte_Color_Control( $wp_customize, $option['id'], $settings )
				);
			}
...
/** Sanitize Color */
function arte_sanitize_color_field( $color ) {
...
/** Sanitize Dropdown */
function arte_sanitize_dropdown_field( $input ) {
	if( is_numeric( $input ) ) {
        return intval( $input );
...
/** Sanitize Checkbox */
function arte_sanitize_checkbox_field( $input ) {
	return is_bool( $input ) ? $input : null;
}

$arte_customizer = new ArteCustomizer();
