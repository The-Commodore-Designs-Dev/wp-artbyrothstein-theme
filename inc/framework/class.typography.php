...
class ArteFont {
	...
	public static function font_transient(){
		$font_array = get_transient( 'arte_google_font_list' );

		if ( $font_array === false || empty( $font_array ) ) {

			$font_array = json_decode( get_option( 'arte_google_font_list' ), true );

			if( empty( $font_array ) ){
				ArteLoadFonts::theme_switch( 'manual', 'trigger' );
			}

			$font_array = json_decode( get_option( 'arte_google_font_list' ), true );

			set_transient( 'arte_google_font_list', $font_array, 60 * 60 * 24 * 30 );
		}
...
class ArteLoadFonts {
    ...
    add_filter( 'arte_fonts_array', array( $this, 'generate_fonts_array' ), 10, 2 );
		add_filter( 'arte_pre_fonts_array', array( $this, 'pre_fonts_array' ), 10, 1 );

	}

	function pre_fonts_array( $tag ){

		$options = apply_filters( 'arte_theme_mods', array() );

		if( ! isset( $options[$tag]['default'] ) ) return;
		...
		return array(
			...
			$font[1],
			$font[2],
			isset( $font[3] ) ? $font[3] : null,
			isset( $font[4] )? $font[4] : null
		);
	}
...
$arte_typography = new ArteLoadFonts();
